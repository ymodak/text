...
#include "tensorflow_text/core/kernels/whitespace_tokenizer_config_util.h"
...
WhitespaceTokenizer::Create(const absl::string_view config) {
  // There are 8 bits per character, and each is used for a different codepoint.
  int max_codepoint = config.length() * 8;
  WhitespaceTokenizer tokenizer(config, max_codepoint);
  return tokenizer;
}
...
void WhitespaceTokenizer::Tokenize(const absl::string_view input,
  ...
  while (position < input_size) {
    ...
    if (IsWhitespace(config, max_codepoint, codepoint)) {
      if (inside_token) {
